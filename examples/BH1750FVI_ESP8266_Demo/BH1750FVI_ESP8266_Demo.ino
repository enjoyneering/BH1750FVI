...
/*
    ...
    - apart from the hardware FIFO (128-bytes for TX & RX), Serial has an
      additional customizable 256-byte RX buffer. For transmit-only
      operation, the 256-byte RX buffer can be switched off to save RAM by
      passing mode "SERIAL_TX_ONLY" to "Serial.begin()"
    - to chage RX buffer size use "Serial.setRxBufferSize(size)", should be
      called before "Serial.begin()", default 256-byte
*/
/**************************************************************************/
void setup()
{
  ...
  while (myBH1750.begin() != true)                         //for ESP-01 use GPIO0=SDA & GPIO2=SCL, "myBH1750.begin(0, 2)"
  {
